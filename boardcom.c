...
#define PIN_RCLK_PORT  GPIOA
#define PIN_RCLK_PIN   GPIO_Pin_6   // 74HC595 RCLK / LATCH  - D12
...
void gpio_init_74hc595(void) {
    ...
    GPIO_InitStruct.GPIO_Pin = PIN_SER_PIN | PIN_RCLK_PIN | PIN_OE_PIN;
    GPIO_Init(GPIOA, &GPIO_InitStruct);

    // Init SRCLK on GPIOB
    GPIO_InitStruct.GPIO_Pin = PIN_SRCLK_PIN;
 ...
 *----------------------------------------------------------------------------*/
int main(void) {
    // UART for printf (like your other exercises)
    uart_init(9600);

    // Initialize ADC on PA0/PA1 using your existing helper
    ADC_setup_PA();

    // Initialize 74HC595 GPIO pins
    gpio_init_74hc595();

    // Small startup delay + banner
    delay_ms(200);
    printf("74HC595 -> L293D analog monitor (STM32F302R8, Nucleo)\r\n");

    while (1) {
        // COAST: IN1=0, IN2=0  -> both outputs ~0 V
        shift595Write(PAT_COAST);
        delay_ms(SETTLE_MS);
        reportState("COAST ");
        delay_ms(PAUSE_MS);

        // FORWARD: IN1=1, IN2=0 -> OUT1 ~ high, OUT2 ~ 0 V
        shift595Write(PAT_FWD);
        delay_ms(SETTLE_MS);
        reportState("FWD   ");
        delay_ms(PAUSE_MS);

        // REVERSE: IN1=0, IN2=1 -> OUT1 ~ 0 V, OUT2 ~ high
